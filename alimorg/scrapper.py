...
# global map
#----------------------------------
surah_map={1:7,2:286,3:200,4:176,5:120,6:165,7:206,8:75,9:129,10:109,11:123,12:111,13:43,14:52,15:99,16:128,17:111,18:110,19:98,20:135,
...
elem_xpath='.//div[@class="content"]'

def main():
    # launch driver
    driver=launchBrowser(Debug=False)
    # collect according to ayat number 
    surah=[]
    ayah=[]
    tafsir=[]
    for k,v in surah_map.items():
        print("scrapping surah:",k)
        for i in tqdm(range(v+1)):
            url=f"https://www.alim.org/quran/tafsir/ibn-kathir/surah/{k}/{i}/"
            driver.get(url)
            if check(driver,elem_xpath):
                elem=driver.find_element_by_xpath(elem_xpath)
                surah.append(k)
                ayah.append(i+1)
                tafsir.append(elem.text)
        # for saving and error handle
        df=pd.DataFrame({"Surah":surah,"Ayah":ayah,"Tafsir":tafsir})
        df.to_csv("tafsir_ibn_kathir.csv",index=False)
